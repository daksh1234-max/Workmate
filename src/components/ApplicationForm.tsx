...
export default function ApplicationForm({ isOpen, onClose, onSuccess, job }: ApplicationFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Emit socket notification to contractor
      if (data.contractorId && data.application) {
        const socketUrl = process.env.NEXT_PUBLIC_SOCKET_URL || 'http://localhost:3000';
        const socket = io(socketUrl);
        
        socket.emit('job-application', {
          contractorId: data.contractorId,
          jobTitle: data.application.job.title,
          labourerName: data.application.labourer.name,
          applicationId: data.application.id,
          jobId: data.application.job.id
        });
