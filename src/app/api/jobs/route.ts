...
export async function GET(request: NextRequest) {
  try {
    ...
    // Handle skill filter with comma-separated skills (OR logic)
    if (skill) {
      const skills = skill.split(',').map(s => s.trim()).filter(s => s.length > 0);
      if (skills.length > 1) {
        // Multiple skills: use OR
        if (location) {
          // Both location and multiple skills: location AND (skill1 OR skill2 OR ...)
          where.AND = [
          ...
          // Multiple skills, no location: just OR
          where.OR = skills.map((s: string) => ({
            skill: { contains: s }
          }));
        }
        ...
        // Single skill
        if (location) {
          // Both location and single skill: use AND explicitly
          where.AND = [
            { location: { contains: location } },
            { skill: { contains: skills[0] } }
          ];
        } else {
          // Single skill, no location
          where.skill = { contains: skills[0] };
        }
      }
    }
    
    // Clean up: if we used AND, remove location and skill from top level
    if (where.AND) {
