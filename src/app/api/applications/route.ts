...
export async function POST(request: NextRequest) {
  try {
    ...
    // Emit socket notification to contractor (will be handled by client-side socket)
    // Note: Socket.io cannot be directly used in API routes, so we'll rely on client-side emission
    // The ApplicationForm component should emit the socket event after successful POST

    return NextResponse.json({
      message: 'Application submitted successfully',
      application,
      contractorId: application.job.contractor.id // Return contractor ID for socket emission
    }, { status: 201 });
